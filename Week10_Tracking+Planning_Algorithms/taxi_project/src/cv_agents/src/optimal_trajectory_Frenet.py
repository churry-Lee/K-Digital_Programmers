...
def calc_frenet_paths(si, si_d, si_dd, sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, opt_d):
    ...
    DF_SET = np.array([LANE_WIDTH/2.5, -LANE_WIDTH/2.5])
...
#        print('fplist[{}].d: {}'.format(i, fplist[i].d[i]))

    return fplist

def collision_check(fp, obs, mapx, mapy, maps):
    COL_CHECK = 1.2 # collision check distance [m] 0.25
...
def check_path(fplist, obs, mapx, mapy, maps):
    ...
    # debuging
    if speed:
        print('Speed fail: {}'.format(speed))
    if acc:
        print('Acc fail: {}'.format(acc))
    if curv:
        print('Curvature fail: {}'.format(curv))
    if collision:
        print('Collision fail: {}'.format(collision))

    print('ok_index: {}'.format(ok_ind))
...
#    print('before check: {}'.format(len(fplist)))
    fplist = check_path(fplist, obs, mapx, mapy, maps)
#    print('after check: {}'.format(len(fplist)))
